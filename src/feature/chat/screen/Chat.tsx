...
const Chat = () => {
  ...
  const scrollToBottom = (checkingBottom: boolean) => {
    ...
    console.log(bubbleContainer.scrollTop, bubbleContainer.scrollHeight, bubbleContainer.clientHeight);

    if (!checkingBottom) {
      bubbleContainer.scrollTop = bubbleContainer.scrollHeight - bubbleContainer.clientHeight;
      return;
    }
    const isBottom = bubbleContainer.scrollTop + bubbleContainer.clientHeight - bubbleContainer.scrollHeight;
    // -24는 textarea의 한줄 높이
